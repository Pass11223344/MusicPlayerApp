...
class recentlyPlayedState extends State<recentlyPlayed> with SingleTickerProviderStateMixin{
 ...
 late myPageController _myPageController ;
      ...
      body:PopScope(
        ...
        child:   TabBarView(
          ...
          children: [
            Obx((){
              ...
              Padding(padding: EdgeInsets.only(bottom: 60),
                child:  ListView.builder(
                    itemExtent: 60,
                    itemCount: _myPageController.recentlyPlayedSongs.length,
            ...
            Obx((){
              if(_myPageController.recentlyPlayedSongSheets.length==0&&currentTab==1){
                getData("songSheet");
              }
              return _myPageController.recentlyPlayedSongSheets.length!=0?
              ListView.builder(
                  itemExtent: 70,
                  itemCount: _myPageController.recentlyPlayedSongSheets.length,
                  itemBuilder: (context,index){
                    return
            ...
            Obx((){
              if(_myPageController.recentlyPlayedAlbums.length==0&&currentTab==2){
                getData("album");
              }
              return _myPageController.recentlyPlayedAlbums.length!=0?
              ListView.builder(
                  itemExtent: 70,
                  itemCount: _myPageController.recentlyPlayedAlbums.length,
                  itemBuilder: (context,index){
    ...
    switch(type){
      case "song":
        dioRequest.executeGet(url: "/record/recent/song").then((value){
          ...
          if(value!="error"){
            _myPageController.recentlyPlayedSongs = ( value['list'] as List<dynamic>).map((json)=>SongListBean.fromJson(json['data'])).toList();
      ...
      case "songSheet":
        dioRequest.executeGet(url: "/record/recent/playlist").then((value){
          if(value!="error"){
            _myPageController.recentlyPlayedSongSheets = ( value['list'] as List<dynamic>).map((json)=>ShowSongSheet(json['playTime'], json['data']['id'], json['data']['name'],json['data']['coverImgUrl'] , UserInfoBean.fromJson(json['data']['creator']))).toList();
          }
        });
        break;
      case "album":
        dioRequest.executeGet(url: "/record/recent/album").then((value){
          if(value!="error"){
            _myPageController.recentlyPlayedAlbums = ( value['list'] as List<dynamic>).map((json){
  ...
  _SongListItem(int index){
    return ListTile(
      title: Text(_myPageController.recentlyPlayedSongs![index].name,maxLines: 1,),
      titleTextStyle: const TextStyle(fontSize: 16,color: Colors.black,
          overflow: TextOverflow.ellipsis,fontWeight: FontWeight.bold),
      subtitle:Text("${ Utils.getSubTitle(_myPageController.recentlyPlayedSongs![index].ar)}-${_myPageController.recentlyPlayedSongs![index].al.name!=null?_myPageController.recentlyPlayedSongs![index].al.name:""}",maxLines: 1,) ,
      ...
      onTap: (){
        if( pageController.lastId==_myPageController.recentlyPlayedSongs[index].id) {
          return;
        }
  ...
  _SongSheetListItem(int index){
    return ListTile(
      leading: getSquareImg(Url: _myPageController.recentlyPlayedSongSheets[index].coverImgUrl, width: 55 ,height: 55,),
      title: Text( _myPageController.recentlyPlayedSongSheets[index].name,maxLines: 1,),
      titleTextStyle: const TextStyle(fontSize: 16,color: Colors.black,
          overflow: TextOverflow.ellipsis,fontWeight: FontWeight.bold),
      subtitle:Text("by ${_myPageController.recentlyPlayedSongSheets[index].creator.nickname}",maxLines: 1,) ,
      subtitleTextStyle:const TextStyle(fontSize: 12,color: Colors.grey,
          overflow: TextOverflow.ellipsis),
      trailing:  Text(Utils.formatDate(_myPageController.recentlyPlayedSongSheets[index].playTime),style: const TextStyle(fontSize: 12,color: Colors.grey,
          overflow: TextOverflow.ellipsis)),
  ...
  _AlbumListItem(int index){
    return ListTile(
      leading: getSquareImg(Url: _myPageController.recentlyPlayedAlbums![index].album.picUrl, width: 55 ,height: 55,),
      title: Text( _myPageController.recentlyPlayedAlbums[index].album.name,maxLines: 1,),
      titleTextStyle: const TextStyle(fontSize: 16,color: Colors.black,
          overflow: TextOverflow.ellipsis,fontWeight: FontWeight.bold),
      subtitle:Text(Utils.getSubTitle(_myPageController.recentlyPlayedAlbums[index].album.artists),maxLines: 1,) ,
      subtitleTextStyle:const TextStyle(fontSize: 12,color: Colors.grey,
          overflow: TextOverflow.ellipsis),
      trailing:  Text(Utils.formatDate(_myPageController.recentlyPlayedAlbums[index].playTime!),style: const TextStyle(fontSize: 12,color: Colors.grey,
          overflow: TextOverflow.ellipsis)),
      onTap: () async{
        // setState(() {
        //   jump_page = true;
