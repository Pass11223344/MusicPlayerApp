...
class purchasedPageState extends State<purchasedPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          backgroundColor: Colors.white,
          title: Text(
              "已购单曲${_myPageController.showSongs != null ? "($songCounts)" : ""}"),
          centerTitle: true,
          bottom: PreferredSize(
              ...
              child: Container(
                ...
                child: Container(
                  ...
                  padding:
                      EdgeInsets.only(left: 12, bottom: 6, right: 6, top: 6),
          ...
          child: Padding(
              ...
              child: NotificationListener<ScrollNotification>(
                  ...
                  child: Stack(
                    children: [
                      Obx(() {
                        return _myPageController.showSongs.length != 0
                            ? CustomScrollView(
                                slivers: [
                                  SliverFixedExtentList.builder(
                                      itemExtent: 150,
                                      itemCount:
                                          _myPageController.showSongs.length,
                                      itemBuilder: (context, index) {
                                        return _focusNode.hasFocus
                                            ? _myPageController
                                                        .showSongs[index].name
                                                        .contains(
  ...
  _SongListItem(int index, MediaQueryData mediaQuery) {
    return InkWell(
      onTap: () {
        if (pageController.lastId == _myPageController.showSongs[index].songId)
          return;
        List<Map<String, dynamic>> jsonList =
            _myPageController.showSongs.map((song) => song.toJson()).toList();
      ...
      child: Row(
        children: [
          getSquareImg(
            Url: _myPageController.showSongs[index].picUrl,
            width: 125,
            height: 125,
            isRadius: true,
          ),
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  _myPageController.showSongs[index].name,
                  maxLines: 1,
                ...
                Text(
                  "${_myPageController.showSongs[index].artistName}${_myPageController.showSongs[index].albumName != "" ? " - ${_myPageController.showSongs[index].albumName}" : ""}",
                  maxLines: 1,
                ),
                Text(
                  "已购${_myPageController.showSongs[index].boughtCount}首",
                  style: const TextStyle(
                      fontSize: 12,
                      color: Colors.grey,
                      overflow: TextOverflow.ellipsis),
  ...
  void getData() {
    dioRequest.executeGet(url: "/song/purchased").then((value) {
      if (value != "error") {
        _myPageController.showSongs = (value['list'] as List<dynamic>)
            .map((json) => ShowSong.fromJson(json))
            .toList();
  ...
  void addMoreList() async {
    ...
    var SList = await dioRequest.executeGet(
        url: "/song/purchased",
        params: {"limit": 50, "offset": _myPageController.showSongs.length});

    _myPageController.addShowSongsSongList((SList['list'] as List<dynamic>)
        .map((json) => ShowSong.fromJson(json))
        .toList());
