  ...
  void receiveDataFromAndroid() {
    channel.setMethodCallHandler((call) async {
      ...
      if (method == "to_msgPage") {
        ...
        notificationList =
            list.map((json) => NotificationListBean.fromJson(json)).toList();
    ...
    return Container(
      ...
      child: Column(
        children: [
          ListTile(
            leading:
                getCircularImg(url: notificationList[index].sendUserAvatarUrl),
            ...
            trailing: Text("${notificationList[index].lastMsgTime}",
                style: TextStyle(fontSize: 8, color: Colors.black)),
            onTap: () async {
              channel.invokeMethod("addPage", "chatPage");
              final result = await Navigator.pushNamed(context, "main/chatPage",
                  arguments: {
