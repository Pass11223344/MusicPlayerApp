...
class musicCloudDiskPageState extends State<musicCloudDiskPage> {
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
                musicCloudDiskBean == null
                    ? ""
                    : "  ${musicCloudDiskBean!.size} / ${musicCloudDiskBean!.maxSize}",
                  ...
                  child: NotificationListener<ScrollNotification>(
                    onNotification: (scrollNotification) {
                      if (scrollNotification is ScrollUpdateNotification &&
                        ...
                        if (musicCloudDiskBean!.simpleSong.length !=
                            musicCloudDiskBean!.count) {
  ...
  void getData() {
    dioRequest.executeGet(url: "/user/cloud").then((value) {
      if (value != "error") {
        ...
        musicCloudDiskBean =
            MusicCloudDiskBean(list, size, maxSize, value['count']);
  ...
  void addMoreList() async {
    ...
    setState(() {});
    var SList = await dioRequest.executeGet(url: "/user/cloud", params: {
      "limit": 100,
      "offset": musicCloudDiskBean!.simpleSong.length
    });

    musicCloudDiskBean!.simpleSong.addAll((SList['data'] as List<dynamic>)
        .map((json) => SongListBean.fromJson(json))
        .toList());
  ...
  _SongListItem(int index) {
    return index == musicCloudDiskBean!.simpleSong.length
        ? Container(
            color: Colors.white,
            width: 70,
            height: 70,
            alignment: Alignment.center,
            child: musicCloudDiskBean!.simpleSong.length !=
                    musicCloudDiskBean!.count
        ...
        : ListTile(
            ...
            title: Text(
              musicCloudDiskBean!.simpleSong[index].name,
              maxLines: 1,
            ),
            ...
            subtitle: Text(
              "${musicCloudDiskBean!.simpleSong[index].singerName}${musicCloudDiskBean!.simpleSong[index].al.name != "" ? "-${musicCloudDiskBean!.simpleSong[index].al.name}" : ""}",
              maxLines: 1,
            ),
            ...
            onTap: () {
              if (pageController.lastAlbumId ==
                  musicCloudDiskBean!.simpleSong[index].id) {
                return;
              }
              pageController.lastAlbumId =
                  musicCloudDiskBean!.simpleSong[index].id;
